...
Beacon committees are allocated to each epoch by the protocol in such a way that each slot has the same number of committees. All active validators are distributed equally among these committees, and one committee member is assigned the duty of ``aggregating the signatures of all other validators in the committee that agree on an attestation'' \cite{efglossary}. These beacon committees are disbanded at the end of the epoch and a new randomly allocated set of committees is created for the next epoch. The protocol specifies a minimum size for a committee, as well as a maximum number of committees per slot  \cite{Edgington2023}.

Each slot typically has several committees, sometimes referred to as `subcommittees' \cite{Neuder2023b}, and the same number of subcommittees is allocated to each slot in the epoch, and hence the total number of committees in a slot is a multiple of 32 \cite{Edgington2023}.
...
The rewards for attestations and sync committee are split $\frac{7}{8}$ and $\frac{1}{8}th$ between the validators performing these duties and the proposer that includes the evidence in blocks, respectively.
...
There is a limit on both entry and exits of validators, which is set at 10 per epoch. The validator activation queue has been very full since the implementation of withdrawal of stake, and that is currently around 80,000 with a wait time of approximate 33 days \cite{ea2023q}. 
...
Slashing is the most severe penalty a validator is subjected to and as explained they can accrue a several extra penalties while they wait to exit. However, there are several smaller, less serious `misdemeanours', or failure to perform their duties that can result in penalties for validators. The validator's stake is reduced by the penalty and is burnt, thereby reducing net issuance \cite{Edgington2023}. \\
...
% ---------------------------------------------------------------
According to Vitalik a griefing attack is when a validator acts maliciously inside a consensus mechanism to reduce other validators' revenue even at some cost to themselves to encourage the victims to drop out of the mechanism \cite{Buterin2018c.}
