...
% ---------------------------------------------------------------
\label{rewards}
Validators earn rewards for three activities \cite{Edgington2023}:
\begin{itemize}
\item Attesting to their view of the beacon chain:
	\begin{itemize}
	\item voting for a source checkpoint
	\item voting for a target checkpoing
	\item voting for a chain head block
	\end{itemize}
\item Proposing a block	
\item Signing off on blocks in the sync committee that support light clients
\end{itemize}
...
In general, the largest part of a validator's rewards (84.4\% - timely source votes 21.9\%, timely target votes 40.6\% and timely head votes 21.9\%) come from making attestations, assuming they are included in a beacon chain block \cite{Edgington2023}. 
...
In the long run, validators can expect to earn $32b \texttt{ } Gwei$ per epoch. Note that as the total active balance changes, \textit{b} changes  \cite{Edgington2023}. When we have auto-compounding of stake, it will also affect the expected rewards. However, consolidating validators is not expected to impact on \textit{b}, but the number of active validators will have the largest impact on expected earnings.

When a validator submits a request to exit, they are expected to still perform their duties until they reach their exit epoch. The waiting time to exit depends on the exit queue which is currently (26 July 2023) empty. There is a limit on both entry and exits of validators, which is set at 10 per epoch. The validator activation queue has been very full since the implementation of withdrawal of stake, and that is currently around 80,000 with a wait time of approximate 33 days \cite{ea2023q}. 
...
Contacted Bogdan (devops) from Infura for cost estimates. Had a meeting with Bogdan which was very insightful and he has put me in touch with Kuhan Tharmananthar and Juan Dominguez from Consensys staking, and Abad Mian and Marta Poling from \gls{mm} staking. Bogdan shared a Google sheet with me with estimates of staking entities and numbers of validators around July 2022. (Need to check this a bit more carefully).
I then met with both teams and had good discussions around staking at Consensys. \gls{mm} is targeting integration with fractional liquid staking pools and working closely with a fractional staking provider so that users can easily participate in staking via \gls{mm}.
\info [inline] {Incorporate information provided by Bogdan}
...
For more detailed information on slashing and calculations, please refer to the latest version of Ben Edgington's  \href{https://eth2book.info/capella/part2/incentives/slashing/}{\textit{Upgrading Ethereum}} book \cite{Edgington2023}  which incorporates the updates included in the Capella hard fork. In summary the events that lead to slashing are:
