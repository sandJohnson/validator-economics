...
The probability of selecting a candidate in the first round = 1/n, the probability of the next, given the validator was not selected in the first round, is 1/(n-1) .... 1/1 = $ \frac{1}{716,800!}$ \\
...
The committee selection process is handled in \href{https://eth2book.info/capella/part3/helper/accessors/#def\_get\_next\_sync\_committee\_indices}{\textit{get\_next\_sync\_committee\_indices}} and \href{https://eth2book.info/capella/part3/helper/accessors/#def\_get\_next\_sync\_committee}{\textit{get\_next\_sync\_committee}}  \cite{Edgington2023} shown below: 
...
Therefore, we shuffle the active validator set at each iteration, and select the first validator index in the list. The next step is to see if this validator passes the selection criteria. If it doesn’t, then we shuffle the set again and select a validator from this set and perform the test again. This loop continues until a committee of size 512 is reached. 

In  \textit{get\_next\_sync\_committee()} below the public keys for the list of indices from \textit{get\_next\_sync\_committee()} are retrieved, after which an  aggregate public key (\textit{aggregate\_pubkey}) is created.
...
\hline
& \textbf{Before } & \multicolumn{7}{c|}{\textbf{After}} \\
& \textbf{consolidation} & \multicolumn{7}{c|} {\textbf{ consolidation}} \\
\cline{2-9}
...
``After consolidation'' in Table \ref{tbl:syncscenario} represents the decisions made by each of the stakers regarding the validators they run after the implementation of EIP-7521. 

In our example the stakers decided on the following strategies:
\begin{itemize}
\item \textit{Staker A} - No validators are consolidated into larger validators, i.e. all 300,000 validators have effective balances of 32 ETH, but they will be compounding.
\item \textit{Staker B} - Opted for the maximum number of fully consolidated (2,048 ETH) validators and the remaining validators were consolidated into one 30-fold validator (960 ETH) and the rest were left as single stake (32 ETH).
\item \textit{Staker C} - The strategy here was to divide the total stake (300,000 * 32 ETH) as much as possible equally between the various consolidation options we chose. In other words for each consolidated subset of validators the total stake was roughly equivalent to 50,000 single validators.  
\end{itemize}


We can calculate the probabilities prior to and after EIP-7521 for each of these three staker groups. These probabilities are shown in Table \ref{tbl:syncprobs} on page \pageref{tbl:syncprobs}
...
%\begin{center}
\setlength\tabcolsep{2pt}                % lessen the white space between table cells (default is 6pt)
\begin{tabular}{|l|c||c|c|c|c|c|c||c|}
\hline
\multirow{4}{*}{\rotatebox[origin=c]{90}{\parbox[c]{1cm}{\centering \textbf{Staker}}}} & \textbf{Before } & \multicolumn{7}{c|}{\textbf{After}} \\
& \textbf{EIP-7251} & \multicolumn{7}{c|}{\textbf{EIP-7521}}  \\
\cline{2-2} \cline{3-9} 
 & Single &  Single & 2-fold & 5-fold & 10-fold & 30-fold & Maximum  &  Staker\\
 & $P(select) * P(accept)$   & $P(select) * P(accept)$   & $P(select) * P(accept)$ & $P(select) * P(accept)$  & $P(select) * P(accept)$   & $P(select) * P(accept)$   & $P(select) * P(accept)$    & $P(sync)$\\
\hline
A & $\frac{1}{3} * 1 = 0.\dot{3}$ & $0.755 *  0.016 = 0.012 $ & $0 * 0.031 = 0$ & $0 * 0.078 = 0$ & $0 * 0.156 = 0 $  & $0 * 0.469 = 0$ & $0 * 1 = 0$ &  $0.012$\\ [2pt] \hline

B & $\frac{1}{3} * 1 = 0.\dot{3} $ & $5.0 * 10^{-6} *0.016 = 8.1 * 10^{-8}$ & $0 * 0.031 = 0$ & $0 * 0.078 = 0 $ & $0 * 0.156 = 0 $ & $2.5 * 10^{-6} * 0.469 = 1.2 * 10^{-6}$ & $0.012 * 1 = 0.012$ & $0.012$\\ [2pt] \hline
C & $\frac{1}{3} * 1 = 0.\dot{3}$ & $0.126 * 0.016 = 0.002$ & $0.063 * 0.031 = 0.002$ & $0.0252 * 0.078 = 0.002$ & $0.013 * 0.156 = 0.002$ & $0.004 * 0.469 = 0.002$ & $0.002 * 1 = 0.002$ & $0.012$ \\  [2pt] \hline
\hline
Total &  1  &  0.014  & 0.002 & 0.002 & 0.002 & 0.002 & 0.014 & 0.036   \\
\hline
...
Although the probability of being selected for a fully consolidated validator is less, when they are selected, they will always pass the test to be accepted as a member of the sync committee. On the other hand, the probability of being chosen if the validator is unconsolidated is much higher, but the probability of passing the acceptance test is much lower. We expect that the two will cancel each other out, which is confirmed when we look at the different consolidation strategies employed by 3 stakers having the same total amount of stake. 
...
\begin{tabular}{|l|c||c|c|c|c|c|c|c|}
\hline
\multirow{3}{*}{\rotatebox[origin=c]{90}{\parbox[c]{1cm}{\centering \textbf{Staker}}}} & \textbf{Before } & \multicolumn{6}{c|}{\textbf{After}} & \\
& \textbf{EIP-7251} & \multicolumn{6}{c|}{\textbf{EIP-7521}} & \textbf{Staker} \\
\cline{2-3} \cline{4-8} 
 & Single &  Single & 2-fold & 5-fold  &  10-fold  &   30-fold  & Maximum & \\
%\cline{2} \cline{3} \cline{4} \cline{5} \cline{6} \cline{7}
\hline
A & 170  & 170  & 0 & 0 & 0 & 0 &  0&   \\ [2pt] \hline

B &  170  & 0  &0 &  & 0.013 & 0 & 0.013  & 0 \\ [2pt] \hline

C & 170  & 0.090 &0.016  & 0.006 & 1 & 0.167 &   & \\ [2pt] \hhline{|=|=|=|=|=|=|=|=|=|}
Total & 510  & 1 & 2 & 5 & 10 & 30 & 64 &    \\
 \hline
