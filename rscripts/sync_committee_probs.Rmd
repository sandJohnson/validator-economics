...
3) A random byte is generated for this candidate (note this candidate may have been chosen before & more than likely
   it would have had a different random byte generated)
...
5) Another iteration of the loop is executed until we have chosen enough validators to make up the sync committee

This appears to be very similar at first glance to proposer selection, but the difference here is that 
we are sampling WITH REPLACEMENT. Therefore each validator has a chance of being chosen more than once and the probabilities of being chosen do not change with each draw.
...
#' Scenario 1: 
#' -----------
#' - The total number of validators (n) in the active validator set is less than 255, meaning 
#'   that even if all validators are chosen to participate in the sync committee, we will
#'   still need more participants. Therefore there will be at least (255-n) duplicate entries.
...
# ---------------------------------------------------------------------------------------
```
# Scenario 2a: prior to EIP-7521

- The total number of validators (n) in the active validator set is more than 255 (128*3=384) 
- Three staker groups: A, B, C
- All validators owned by the three staker groups are solo stakers
- A, B, and C have all staked the same amount, i.e. run the same number of validators = 128

```{r}

```

#' Scenario 2b: post EIP-7521
#' -----------
#' - The total number of validators (n) in the active validator set is more than 255 (128*3=384) 
#' - Three staker groups: A, B, C
#' - The validators owned by the three staker groups have been consolidated in different ways
#' - A, B, and C have all staked the same amount, so even though the number of validators owned by each group may differ, 
#'   they all have the same number of virtual validators.
#' 
...
# Pre-EIP-7521
scenario2b_pre_EIP7521_prob <- 1/3          # 128/384 

# We want to visualise the probability that each of the groups have one or more validators in the sync committee
...
scenario2b_post_EIP7521_prob_A <- 128/156   # single stake (unconsolidated)
scenario2b_post_EIP7521_prob_B <- 2/156     # maxEB stake
scenario2b_post_EIP7521_prob_C <- 26/156    # varying consolidation 

# Post-EIP-7521 for four consolidated validator types
scenario2b_post_EIP7521_prob_single  <- 142/156  #  1 x 32 ETH
scenario2b_post_EIP7521_prob_single  <- 5/156    #  2 x 32 ETH
scenario2b_post_EIP7521_prob_5_fold  <- 4/156    #  5 x 32 ETH
scenario2b_post_EIP7521_prob_10_fold <- 2/156    # 10 x 32 ETH
scenario2b_post_EIP7521_prob_maxEB   <- 3/156    # 64 x 32 ETH
